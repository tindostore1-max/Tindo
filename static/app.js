...
// Variables globales para Google Auth
let googleAuthAvailable = false;

// Verificar disponibilidad de Google Auth al cargar
async function verificarGoogleAuth() {
    try {
        const response = await fetch('/auth/google/status');
        const data = await response.json();
        googleAuthAvailable = data.available;
        
        if (googleAuthAvailable) {
            mostrarBotonGoogle();
        }
    } catch (error) {
        console.log('Google Auth no disponible:', error);
        googleAuthAvailable = false;
    }
}

// Mostrar botón de Google en el formulario de login
function mostrarBotonGoogle() {
    const loginForm = document.getElementById('login-form');
    if (!loginForm) return;
    
    // Buscar si ya existe el botón
    const existingButton = document.getElementById('google-login-btn');
    if (existingButton) return;
    
    // Crear botón de Google
    const googleButton = document.createElement('div');
    googleButton.innerHTML = `
        <div class="google-login-section" style="margin-top: 20px; text-align: center;">
            <div style="margin: 20px 0; display: flex; align-items: center; text-align: center;">
                ...
                <span style="padding: 0 15px; color: #cccccc; font-size: 14px;">o</span>
                <hr style="flex: 1; border: none; border-top: 1px solid rgba(255,255,255,0.2);">
            </div>
            <button type="button" id="google-login-btn" onclick="iniciarLoginGoogle()" class="google-login-button">
    ...
    if (submitButton) {
        submitButton.parentNode.insertBefore(googleButton, submitButton);
    }
}
...
function iniciarLoginGoogle() {
    if (!googleAuthAvailable) {
        mostrarAlerta('Google Auth no está disponible', 'error');
        return;
    }
    ...
    if (googleBtn) {
        ...
        googleBtn.innerHTML = '⏳ Conectando con Google...';
...
function inicializarEventos() {
    // Verificar Google Auth al cargar
    verificarGoogleAuth();
